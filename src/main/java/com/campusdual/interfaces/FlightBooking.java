...
public class FlightBooking implements IBooking{
    ...
    this.bookedSeat = "";
        System.out.println("Booked successfully canceled");
    }

    @Override
    public void getTicketDetail() {
    if (!this.bookedSeat.isEmpty()){
        System.out.println("Flight "+ this.flightNumber + "( " + this.company +"), for date " +this.sdf.format(this.bookDate) + " has a booked seat" + this.bookedSeat);
    }else{
